...
async function generateImage(imageConfig, index) {
  console.log(`\n[${index + 1}/${images.length}] Generating: ${imageConfig.filename}`);
  ...
  try {
    ...
    for (const part of parts) {
      if (part.inlineData) {
        ...
        const outName = imageConfig.filename.replace('.webp', `.${ext}`);
        const outPath = path.join('public', outName);
        fs.writeFileSync(outPath, buffer);
        console.log(`  Saved: ${outPath} (${(buffer.length / 1024).toFixed(0)} KB, ${mimeType})`);

        if (ext !== 'webp') {
          const webpPath = path.join('public', imageConfig.filename);
          fs.copyFileSync(outPath, webpPath);
          fs.unlinkSync(outPath);
          console.log(`  Renamed to: ${webpPath}`);
        }
        return true;
      }
    }

    console.error(`  ERROR: No image data in response. Parts: ${JSON.stringify(parts.map(p => Object.keys(p)))}`);
    return false;
  } catch (error) {
...
async function main() {
  console.log('=== Avera Coffee Image Generation ===');
  ...
  for (let i = 0; i < images.length; i++) {
    const success = await generateImage(images[i], i);
    if (success) {
      results.success++;
    } else {
      results.failed++;
      results.failures.push(images[i].filename);
    }
  ...
  console.log('\n=== COMPLETE ===');
  console.log(`Success: ${results.success}/${images.length}`);
